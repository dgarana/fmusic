...
export function WindowTitleBar() {
  ...
  useEffect(() => {
    // Initial state
    window.fmusic.getPlatform().then(setPlatform).catch(() => {});
  ...
  // Don't render anything until we know the platform
  ...
  return (
    <div 
      className={`window-title-bar ${isMac ? 'mac' : ''}`}
      style={isMac ? ({ WebkitAppRegion: 'drag' } as any) : undefined}
    >
      {!isMac && (
        <>
